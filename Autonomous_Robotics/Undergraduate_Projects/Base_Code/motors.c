...
void initPWM (void) {
    ...
    PWMPulseWidthSet (PWM1_BASE, PWM_OUT_0, 350);       // ~--% duty cycle.
    PWMPulseWidthSet (PWM0_BASE, PWM_OUT_2, 350);       // ~--% duty cycle.
...
bool getDistance (float dest, uint8_t xyTurn, bool reset) {
    ...
    if (xyTurn == 0x1) {
        ...
        if (fabs(pose.ot) >= dest*50) {
            /* Return true, save desired pose value. */
            retVal = true;
            gPose.ot = gPose.ot/50 + previousPose;                 // Needs multiplier of 50 for increased accuracy. Remove this multiplier when saving pose.
    ...
    while (true) {
        if (timeout_flag == true) {
            ...
            if (getDistance(distance, 0x1, false) || obstacle != 0x0) {                                  // 0x1 tells getDistance function to check X pose.
                ...
                if (obstacle == 0x1) {
                    startStopTimer(0);
                    driveStraight((direction*-1), 5);
                    turnRobot(obstacle, 1.6);
                    startStopTimer(1);
                } else if (obstacle == 0x2) {
                    startStopTimer(0);
                    driveStraight((direction*-1), 5);
                    turnRobot(obstacle, 1.6);
                    startStopTimer(1);
...
void setSpeed (uint8_t mag) {
    ...
    if ((350*magnitude) < 150)
        wheel_speed = 150;
    else if ((350*magnitude) > 350)
        wheel_speed = 350;
    else
        wheel_speed = 350 * magnitude;
}
