...
/* Interrupt handler for PF4. */
void stopMovingSon (void) {
    /* Set flag. */
    stopMoving = true;
...
/* Enable interrupt on PORTF to stop the motor control from moving robot. */
void initInterruptPortF (void) {
    /* Disable interrupt. */
    ...
    GPIOIntRegister (GPIO_PORTF_BASE, stopMovingSon);
    ...
    /* IntMasterEnable() called in initMotors function. */
}
...
bool getDistance (float dest, uint8_t xyTurn, bool reset) {
    ...
    if (xyTurn == 0x1) {
        ...
        if (fabs(pose.xt) >= dest || stopMoving == true) {
        ...
        if (fabs(pose.ot) >= dest*45 || stopMoving == true) {
            /* Return true, save desired pose value. */
            retVal = true;
            gPose.ot = gPose.ot/45 + previousPose;                 // Needs multiplier of 50 for increased accuracy. Remove this multiplier when saving pose.
...
void driveStraight (uint8_t direction, float distance) {
    ...
    PWMPulseWidthSet (PWM0_BASE, PWM_OUT_2, wheel_speed);       // ~--% duty cycle.
